...
import { allNews, trendingTags } from '../newsData'; // সেন্ট্রাল ফাইল থেকে ডাটা ইমপোর্ট
...
// 🎯 অবজেক্ট ডিস্ট্রাকচারিং করে onCategoryClick, onNewsClick এবং onLocationSearch রিসিভ করা হলো
export default function Home({ onCategoryClick, onNewsClick, onLocationSearch }) {
    const safeAllNews = Array.isArray(allNews) ? allNews : [];

    const sortNewsByCreatedAtDesc = (items) => {
        return [...items]
            .map((item, index) => {
                const createdAtTimestamp = item?.createdAt ? new Date(item.createdAt).getTime() : Number.NEGATIVE_INFINITY;

                return {
                    item,
                    index,
                    createdAtTimestamp: Number.isFinite(createdAtTimestamp) ? createdAtTimestamp : Number.NEGATIVE_INFINITY,
                };
            })
            .sort((left, right) => {
                if (right.createdAtTimestamp !== left.createdAtTimestamp) {
    ...
    const sortedAllNews = sortNewsByCreatedAtDesc(safeAllNews);
    ...
    const getCategoryNews = (category, extraFilter = () => true) => (
        sortedAllNews.filter(news => normalize(news.category) === normalize(category) && extraFilter(news))
    );
    ...
    const bigFeaturedNews = latestNewsList[0] || null;
    ...
    const politicsLead = politicsNewsList[0] || null;
    ...
    const worldLeadNews = worldNewsList[0] || null;
    ...
    const BangladeshLead = BangladeshNewsList[0] || null;
    ...
    const economyLead = economyNewsList[0] || null;
    ...
    const bigFactCheck = factCheckList[0] || null;
    ...
    const jobLeadNews = jobNewsList[0] || null;
    ...
    const bigVideo = videoNewsList[0] || null;
    const rightAnalysisVideo = videoNewsList.find(news => news.videoType === "analysis" && news.id !== bigVideo?.id) || null;
    ...
    const sportsLeadNews = sportsNewsList[0] || null;
    ...
    const entLeadNews = entertainmentNewsList[0] || null;
    ...
    const opinionLeadNews = opinionNewsList[0] || null;
    ...
    return (
        <div className="home-page">
            ...
            {/* 8. সর্বাধিক পঠিত ও ফ্যাক্টচেক সেকশন */}
            <MostRead
