...
export default function LifestyleSection({ mainLifestyle, sidebarLifestyle, onSeeAllClick, onNewsClick }) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4">
          {mainLifestyle?.map((item) => (
            <article key={item.id} onClick={() => handleNewsClick(item)} className="bg-white rounded-3xl shadow-sm overflow-hidden group cursor-pointer border border-gray-50">
        ...
        <aside className="lg:col-span-1">
          <div className="grid grid-cols-2 gap-2">
            {sidebarLifestyle?.map((item) => (
              <article key={item.id} onClick={() => handleNewsClick(item)} className="flex flex-col group cursor-pointer">
